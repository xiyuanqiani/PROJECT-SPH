  ...
  <div class="cart">
      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(cart,index) in cartInfoList" :key=cart.skuId>
          ...
          <li class="cart-list-con5">
            ...
            <input autocomplete="off" type="text" :value="cart.skuNum" minnum="1" class="itxt" @click="handler('change',$event.target.value*1,cart)">
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input class="chooseAll" type="checkbox" :checked="isAllChecked&&cartInfoList.length>0" @click="changeAllChecked($event)">
        <span>全选</span>
      </div>
      <div class="option">
  ...
  export default {
    ...
    methods: {
      ...
      handler:throttle(async function(type,disNum,cart){
        // type为了区分这三个元素，disNum +变化量(1) -变化量(-1) input最终的个数 cart(id)哪一个产品
        switch(type){
          case 'add':
            disNum = 1;
            break;
          case 'minus':
            // 产品个数大于1才传给服务器-1，如果小于1，传0（原封不动）
            // 点击过快会变0！！！！
    ...
    computed:{
      ...
      // 计算产品总价(可过滤未选中的且reduce实现)
      totalPrice(){
        let filterCart = this.cartInfoList.filter(item=>
          item.isChecked==1
        )
        console.log(filterCart);
        let result = filterCart.reduce((sum,item)=>{
